  ...
  if(typeof gon != 'undefined') {
    if(typeof gon.category_user_select != 'undefined') {
      ...
      if (current_url == edit_url){
        // 商品の初期画像を表示する。
        product_image_data = $("#preview input[type= 'hidden']")
      ...
      // ドロップした画像を$upload_filesに代入する。
        ...
        // 選択した画像の削除
        $(".main-contents__conteiner #preview, .main-contents__conteiner #preview--next").on('click', ".image-remove-btn",function(e){
      ...
      $('.edit_item').on('submit', function(e){
        ...
        for (i = 0; i < upload_files.length; i++){//upload_files内のデータを取り出し、それぞれにキーを分配する。
          ...
          if (typeof upload_files[i][0] != "undefined"){
            formdata.append(ajax_files[i], upload_files[i][0]);
          }
          else {
            formdata.append(ajax_files[i], upload_files[i].id);
          }
        };
        url = window.location.protocol + '//' + window.location.host + '/items/' + gon.category_user_select.id
        type = "patch"

class ItemsController < ApplicationController
  ...
  def create
    ...
    if @item.save 
      @item_image = ItemImage.create(image_params)
...
def items_params
  ...
  @params_item = params.require(:item).permit(:name, :detail, :condition, :delivery_cost, :delivery_prefecture, :days_to_ship, :delivery_method, :price).merge(user_id: 1, sales_condition: 1, category_id: @params_category[:id], brand_id: @params_brand)
